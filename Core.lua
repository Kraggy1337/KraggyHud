if not _G.KraggyHUD then
	_G.KraggyHUD = {}
	KraggyHUD.mod_path = ModPath
	KraggyHUD.save_path = SavePath
	KraggyHUD.settings_path = KraggyHUD.save_path .. "KraggyHUD_v2.json"
	KraggyHUD.tweak_file = "KraggyHUDTweakData.lua"
	KraggyHUD.LOG_MODE = { error = true, warning = true, info = false }		-- error, info, warning or all
	KraggyHUD.identifier = string.match(KraggyHUD.mod_path, "(%w+)[\\/]$") or "KraggyHUD"

	KraggyHUD.settings = {}
	KraggyHUD.tweak_data = {}

	KraggyHUD.hook_files = KraggyHUD.hook_files or {
		...
		if msg_type and self.LOG_MODE[tostring(msg_type)] then
			if type(text) == "table" or type(text) == "userdata" then
				...
				if not text[1] or type(text[1]) ~= "string" then
					log(string.format("[KraggyHUD] %s:", string.upper(type(msg_type))))
					log_table(text)
					return
				else
			...
			log(string.format("[KraggyHUD] %s: %s", string.upper(msg_type), text))
		end
	end

	function KraggyHUD:Load()
	...
	function KraggyHUD:createDirectory(path)
		local current = ""
	...
	function KraggyHUD:getVersion()
        local mod = BLT and BLT.Mods:GetMod(KraggyHUD.identifier or "")
		return tostring(mod and mod:GetVersion() or "(n/a)")
	end

	function KraggyHUD:SafeTableConcat(tbl, str)
		local res
		for i = 1, #tbl do
	...
	function KraggyHUD:getSetting(id_table, default)
	...
	function KraggyHUD:setSetting(id_table, value)
		local entry = self.settings
	...
	function KraggyHUD:getColor(name, ...)
		if self.tweak_data and type(name) == "string" then
			for i, data in ipairs(self:getTweakEntry("color_table", "table")) do
				if name == data.name then
	...
	function KraggyHUD:getTweakEntry(id, val_type, default)
	...
	function KraggyHUD:makeOutlineText(panel, bg, txt)
		bg.name = nil
		local bgs = {}
		for i = 1, 4 do
			table.insert(bgs, panel:text(bg))
		end
		KraggyHUD:refreshOutlinePos(bgs, txt)
		return bgs
	end

	function KraggyHUD:refreshOutlinePos(bgs, txt)
		bgs[1]:set_x(txt:x() - 1)
		bgs[1]:set_y(txt:y() - 1)
		bgs[2]:set_x(txt:x() + 1)
		bgs[2]:set_y(txt:y() - 1)
	...
	function KraggyHUD:setOutlineText(bgs, text, show)
	...
	function KraggyHUD:setOutlineFontSize(bgs, size)
		for _, bg in pairs(bgs) do
			bg:set_font_size(size)
		end
	end

	function KraggyHUD:truncateNameTag(name)
	...
	if not KraggyHUD.tweak_path then		-- Populate tweak data
		local tweak_path = string.format("%s%s", KraggyHUD.save_path, KraggyHUD.tweak_file)
		if not io.file_is_readable(tweak_path) then
			tweak_path = string.format("%s%s", KraggyHUD.mod_path, KraggyHUD.tweak_file)
		end
		if io.file_is_readable(tweak_path) then
			dofile(tweak_path)
			KraggyHUD.tweak_data = KraggyHUDTweakData:new()
		else
			KraggyHUD:print_log(string.format("Tweak Data file couldn't be found! (%s)", tweak_path), "error")
		end
	end

	-- Table with all menu IDs
	KraggyHUD.menu_ids = KraggyHUD.menu_ids or {}

	--callback functions to apply changed settings on the fly
	if not KraggyHUD.apply_settings_clbk then
		KraggyHUD.apply_settings_clbk = {
			["CustomHUD"] = function(setting, value)
				if managers.hud and managers.hud.change_hud_setting then
					...
					managers.hud:change_hud_setting(type, setting, KraggyHUD:getColor(value) or value)
				end
			end,
			["HUDList"] = function(setting, value)
				if managers.hud and HUDListManager then
					if setting[1] == "BUFF_LIST" and setting[2] ~= "show_buffs" then
						managers.hud:change_bufflist_setting(tostring(setting[#setting]), KraggyHUD:getColor(value) or value)
					elseif setting[1] == "RIGHT_LIST" and setting[2] == "SHOW_PICKUP_CATEGORIES" then
						managers.hud:change_pickuplist_setting(tostring(setting[#setting]), KraggyHUD:getColor(value) or value)
					else
						managers.hud:change_list_setting(tostring(setting[#setting]), KraggyHUD:getColor(value) or value)
					end
				end
			end,
			...
			["DrivingHUD"] = function(setting, value)
				if managers.hud and managers.hud.change_drivinghud_setting then
					managers.hud:change_drivinghud_setting(tostring(setting[#setting]), KraggyHUD:getColor(value) or value)
				end
			end,
			["GADGETS"] = function(setting, value)
				if managers.hud and managers.hud.change_hud_setting and #setting >= 4 then
					WeaponGadgetBase.update_theme_setting(setting[1], setting[2], setting[3], setting[4], KraggyHUD:getColor(value) or value)
				end
			end,
		}
	end

	KraggyHUD:Reset()	-- Populate settings table
	KraggyHUD:Load()	-- Load user settings

	-- Create Ingame Menus
	dofile(KraggyHUD.mod_path .. "OptionMenus.lua")	-- Menu structure table in seperate file, in order to not bloat the Core file too much.
	local menu_options = KraggyHUD.options_menu_data

	-- Setup and register option menus
	Hooks:Add("MenuManagerSetupCustomMenus", "MenuManagerSetupCustomMenus_KraggyHUD", function( menu_manager, nodes )
	...
	Hooks:Add("MenuManagerPopulateCustomMenus", "MenuManagerPopulateCustomMenus_KraggyHUD", function(menu_manager, nodes)
		-- Called on setting change
		local function change_setting(setting, value)
			if KraggyHUD:getSetting(setting, nil) ~= value and KraggyHUD:setSetting(setting, value) then
				KraggyHUD:print_log(string.format("Change setting: %s = %s", KraggyHUD:SafeTableConcat(setting, "->"), tostring(value)), "info")	-- Change type back!
				KraggyHUD.settings_changed = true

				local script = table.remove(setting, 1)
				if KraggyHUD.apply_settings_clbk[script] then
					KraggyHUD.apply_settings_clbk[script](setting, value)
				end
			end
		end
			...
			MenuCallbackHandler[visual_clbk_id] = function(self, item)
				for _, req in ipairs(data.visible_reqs or {}) do
					if type(req) == "table" then
						local a = KraggyHUD:getSetting(req.setting, nil)
						if req.equal then
							if a ~= req.equal then
								return false
							end
		...
		local update_visible_clbks = "KraggyHUD_update_visibility"
		...
		local create_item_handlers = {
			menu = function(parent_id, offset, data)
				if not table.contains(KraggyHUD.menu_ids, data.menu_id) then
					table.insert(KraggyHUD.menu_ids, data.menu_id)
				end
			end,
			...
			multi_choice = function(menu_id, offset, data, value)
				...
				if data.add_color_options then
					...
					for i, item in pairs(menu._items_list) do
						if item:parameters().name == id then
							item:clear_options()
							for k, v in ipairs(KraggyHUD:getTweakEntry("color_table", "table") or {}) do
								if data.add_rainbow or v.name ~= "rainbow" then
									local color_name = managers.localization:text("KraggyHUD_colors_" .. v.name)
		...
		local function populate_menu(menu_table, parent_id)
			...
			for i, data in ipairs(menu_table) do
				local value = data.value and KraggyHUD:getSetting(data.value, nil)
	...
	Hooks:Add("MenuManagerBuildCustomMenus", "MenuManagerBuildCustomMenus_KraggyHUD", function(menu_manager, nodes)
		local back_clbk = "KraggyHUD_back_clbk"
		local focus_clbk = "KraggyHUD_focus_clbk"
		local reset_clbk = "KraggyHUD_reset_clbk"

		-- Add menu back callback
		MenuCallbackHandler[back_clbk] = function(node)
			if KraggyHUD.settings_changed then
				KraggyHUD.settings_changed = nil
				KraggyHUD:Save()
				KraggyHUD:print_log("Settings saved!", "info")
			end
		end
		...
		MenuCallbackHandler[reset_clbk] = function(self, item)
			local menu_title = managers.localization:text("KraggyHUD_reset_options_title")
			local menu_message = managers.localization:text("KraggyHUD_reset_options_confirm")
			local menu_buttons = {
				[1] = {
					...
					callback = function(self, item)
						...
						for i, menu_id in ipairs(KraggyHUD.menu_ids) do
							local menu = MenuHelper:GetMenu(menu_id)
							if menu then
								for __, menu_item in ipairs(menu._items_list) do
									...
									if menu_item.set_value and setting then
										local value = KraggyHUD:getSetting(setting, nil)
										if value ~= nil then
											local script = table.remove(setting, 1)
											if KraggyHUD.apply_settings_clbk[script] then
												KraggyHUD.apply_settings_clbk[script](setting, value)
						...
						KraggyHUD.settings_changed = true
						KraggyHUD:print_log("Settings resetted!", "info")
					end,
				},
				[2] = {
	...
	Hooks:Add("LocalizationManagerPostInit", "LocalizationManagerPostInit_KraggyHUD", function(loc)
        local loc_path = KraggyHUD.mod_path .. "loc/"
		if file.DirectoryExists( loc_path ) then
			...
			if KraggyHUD:getSetting({"INVENTORY", "USE_REAL_WEAPON_NAMES"}, false) then
				loc:load_localization_file(loc_path .. "RealWeaponNames.json")
			end
		else
			KraggyHUD:print_log("Localization folder seems to be missing!", "error")
		end

		local localized_strings = {}
		localized_strings["cash_sign"] = KraggyHUD:getTweakEntry("CASH_SIGN", "string", "$")

		-- Hide Skip Message, when auto skip blackscreen is active
		if KraggyHUD:getSetting({"SkipIt", "SKIP_BLACKSCREEN"}, false) then
...
if MenuNodeMainGui then
	Hooks:PostHook( MenuNodeMainGui , "_add_version_string" , "MenuNodeMainGuiPostAddVersionString_KraggyHUD" , function( self )
		if alive(self._version_string) then
			self._version_string:set_text("Payday 2 v" .. Application:version() .. " | KraggyHUD v" .. KraggyHUD:getVersion())
		end
	end)
end

if MenuItemMultiChoice then
	Hooks:PostHook( MenuItemMultiChoice , "setup_gui" , "MenuItemMultiChoicePostSetupGui_KraggyHUD" , function( self, node, row_item )
...
if EconomyTweakData then
	...
	Hooks:PostHook(EconomyTweakData, "create_weapon_skin_market_search_url" ,"KraggyHUD_EconomyTweakDataPostCreateWeaponSkinMarketSearchUrl", function(self, weapon_id, cosmetic_id)
...
if RequiredScript then
	...
	if KraggyHUD.hook_files[requiredScript] then
		for __, file in ipairs(KraggyHUD.hook_files[requiredScript]) do
			dofile( KraggyHUD.mod_path .. "lua/" .. file )
		end
	end
end