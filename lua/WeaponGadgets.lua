local print_info = print_info or function(...) KraggyHUD:print_log(string.format(...), "info") end
...
if RequiredScript == "lib/units/weapons/weapongadgetbase" then
	...
	WeaponGadgetBase.THEME_SETTINGS = {
		laser = KraggyHUD:getSetting({"GADGETS", "laser"}, nil),
		flashlight = KraggyHUD:getSetting({"GADGETS", "flashlight"}, nil)
	}
...
if string.lower(RequiredScript) == "lib/units/weapons/newraycastweaponbase" then
	...
	function NewRaycastWeaponBase:on_enabled(...)
		...
		if KraggyHUD:getSetting({"GADGETS", "LASER_AUTO_ON"}, true) and not self._init_laser_state then
			self:_setup_laser()
...
elseif string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	...
	function PlayerStandard:_stance_entered(...)
		...
		local rotate_weapon = KraggyHUD:getSetting({"GADGETS", "SHOW_ANGELED_SIGHT"}, true) and sight_id and PlayerStandard.ANGELED_SIGHTS[sight_id]
