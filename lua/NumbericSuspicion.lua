...
function HUDSuspicion:init(...)
	...
	self._suspicion_text_bgs = KraggyHUD:makeOutlineText(_suspicion_text_panel, {
		visible = true,
		text = "",
		valign = "center",
		align = "center",
...
function HUDSuspicion:_animate_detection_text(_suspicion_panel)
	...
	while self._animating_text do
		...
		if dt > 0.01 then
			...
			KraggyHUD:setOutlineText(self._suspicion_text_bgs, text, KraggyHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE_OUTLINE"}, true))
			dt = 0
		end
	end
end
...
function HUDSuspicion:animate_eye(...)
	...
	self:rescale()
	local visibile = KraggyHUD:getSetting({"HUDSuspicion", "SHOW_BARS"}, true)
	...
	if KraggyHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE"}, true) then
...
function HUDSuspicion:rescale()
	local scale = KraggyHUD:getSetting({"HUDSuspicion", "SCALE"}, 0.8)
	if self._scale ~= scale then
		...
		KraggyHUD:setOutlineFontSize(self._suspicion_text_bgs, fontSize)
		...
		KraggyHUD:refreshOutlinePos(self._suspicion_text_bgs, suspicion_text)
		self._scale = scale
	end
	if KraggyHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE"}, true) and self._animating_text and not self._text_animation then
		self._text_animation = self._suspicion_panel:child("suspicion_text_panel"):animate(callback(self, self, "_animate_detection_text"))
	end
end