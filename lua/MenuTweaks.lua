if string.lower(RequiredScript) == "lib/managers/menumanager" then
	...
	function MenuCallbackHandler:save_lobby_settings(setting, value)
		...
		if setting then
			...
			KraggyHUD:setSetting({"LOBBY_SETTINGS", setting}, value)
		else
			local lobby_settings = KraggyHUD:getSetting({"LOBBY_SETTINGS"}, {})
			for id, value in pairs(lobby_settings) do
				if Global.game_settings[id] ~= nil then
					lobby_settings[id] = Global.game_settings[id]
				end
			end
			KraggyHUD:setSetting({"LOBBY_SETTINGS"}, lobby_settings)
		end
		KraggyHUD:Save()
	end

	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_lobby_job_plan" , "MenuCallbackHandlerPostSaveJobPlan_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("job_plan")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_kicking_option" , "MenuCallbackHandlerPostSaveKickOption_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("kick_option")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_lobby_permission" , "MenuCallbackHandlerPostSavePermission_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("permission")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_lobby_reputation_permission" , "MenuCallbackHandlerPostSaveReputationPermission_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("reputation_permission")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_drop_in" , "MenuCallbackHandlerPostSaveDropInOption_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("drop_in_option")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_team_ai" , "MenuCallbackHandlerPostSaveTeamAIOption_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("team_ai")
		self:save_lobby_settings("team_ai_option")
	end)
	Hooks:PostHook( MenuCallbackHandler , "choice_crimenet_auto_kick" , "MenuCallbackHandlerPostSaveAutoKick_KraggyHUD" , function( self, ... )
		self:save_lobby_settings("auto_kick")
	end)
	Hooks:PostHook( MenuCallbackHandler , "change_contract_difficulty" , "MenuCallbackHandlerPostSaveDifficulty_KraggyHUD" , function( self, item, ... )
		self:save_lobby_settings("difficulty", tweak_data:index_to_difficulty(item:value()))
	end)

	Hooks:PostHook( MenuCallbackHandler , "update_matchmake_attributes" , "MenuCallbackHandlerPostUpdateMatchmakeAttributes_KraggyHUD" , function( self, item, ... )
		self:save_lobby_settings()
	end)

	local KraggyHUD_LOBBY_SETTINGS_LOADED = false
	...
	function MenuCrimeNetContractInitiator:modify_node(original_node, data, ...)
		if not KraggyHUD_LOBBY_SETTINGS_LOADED then
			local lobby_settings = KraggyHUD:getSetting({"LOBBY_SETTINGS"}, {})
			for id, value in pairs(lobby_settings) do
				if Global.game_settings[id] ~= nil then
			...
			KraggyHUD_LOBBY_SETTINGS_LOADED = true
		end

		if data.customize_contract then
			data.difficulty = KraggyHUD:getSetting({"LOBBY_SETTINGS", "difficulty"}, "normal")
...
elseif string.lower(RequiredScript) == "lib/managers/menu/blackmarketgui" then
	...
	function BlackMarketGui:populate_weapon_category_new(data, ...)
		...
		local show_icons = not KraggyHUD:getSetting({"INVENTORY", "SHOW_WEAPON_MINI_ICONS"}, true)
	...
	function BlackMarketGuiSlotItem:init(main_panel, data, ...)
		if KraggyHUD:getSetting({"INVENTORY", "SHOW_WEAPON_NAMES"}, true) then
	...
	function BlackMarketGui:_setup(is_start_page, component_data)
		...
		local inv_name_tweak = KraggyHUD:getSetting({"INVENTORY", "CUSTOM_TAB_NAMES"}, {})
		...
		if self._renameable_tabs and not component_data.is_loadout  and alive(self._panel) then
			...
			legends_panel:text({
				name = "LegendText",
				text = managers.localization:text("KraggyHUD_inv_tab_rename_hint"),
	...
	function BlackMarketGui:rename_tab_clbk(tab, tab_id)
		...
		local inv_name_tweak = KraggyHUD:getSetting({"INVENTORY", "CUSTOM_TAB_NAMES"}, nil)
		if current_tab and tab_data and inv_name_tweak and not self:in_setup()then
			...
			QuickInputMenu:new(managers.localization:text("KraggyHUD_dialog_rename_inv_tab"), managers.localization:text("KraggyHUD_dialog_rename_inv_tab_desc"), prev_name, menu_options, true, {w = 420, to_upper = true, max_len = 15})
...
elseif string.lower(RequiredScript) == "lib/managers/menu/skilltreeguinew" then
	...
	function NewSkillTreeSkillItem:refresh(...)
		...
		if alive(self._skill_panel) and KraggyHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
	...
	function NewSkillTreeTierItem:init(...)
		...
		if KraggyHUD:getSetting({"INVENTORY", "SHOW_SKILL_NAMES"}, true) then
...
elseif string.lower(RequiredScript) == "lib/tweak_data/tweakdata" then
	if tweak_data then
		...
		if Network:is_server() and KraggyHUD:getSetting({"SkipIt", "INSTANT_RESTART"}, false) then
...
elseif string.lower(RequiredScript) == "lib/tweak_data/guitweakdata" then
	...
	function GuiTweakData:init(...)
		...
		self.rename_max_letters = KraggyHUD:getTweakEntry("MAX_WEAPON_NAME_LENGTH", "number", 30)
		self.rename_skill_set_max_letters = KraggyHUD:getTweakEntry("MAX_SKILLSET_NAME_LENGTH", "number", 25)
...
elseif string.lower(RequiredScript) == "lib/states/ingamewaitingforplayers" then
	local SKIP_BLACKSCREEN = KraggyHUD:getSetting({"SkipIt", "SKIP_BLACKSCREEN"}, true)
...
elseif string.lower(RequiredScript) == "lib/managers/menu/stageendscreengui" then
	...
	function StageEndScreenGui:init(...)
		...
		if self._enabled and KraggyHUD:getSetting({"SkipIt", "STAT_SCREEN_SPEEDUP"}, false) and managers.hud then
			managers.hud:set_speed_up_endscreen_hud(5)
		end
	end

	local SKIP_STAT_SCREEN_DELAY = KraggyHUD:getSetting({"SkipIt", "STAT_SCREEN_DELAY"}, 5)
	...
	function StageEndScreenGui:special_btn_pressed(...)
		if not KraggyHUD:getSetting({"SkipIt", "STAT_SCREEN_SPEEDUP"}, false) then
...
elseif string.lower(RequiredScript) == "lib/managers/menu/lootdropscreengui" then
	local SKIP_LOOT_SCREEN_DELAY = KraggyHUD:getSetting({"SkipIt", "LOOT_SCREEN_DELAY"}, 3)
	local AUTO_PICK_CARD = KraggyHUD:getSetting({"SkipIt", "AUTOPICK_CARD"}, true)
	local AUTO_PICK_SPECIFIC_CARD = KraggyHUD:getSetting({"SkipIt", "AUTOPICK_CARD_SPECIFIC"}, 1)
...
elseif string.lower(RequiredScript) == "lib/managers/menu/contractboxgui" then
	...
	function ContractBoxGui:create_character_text(peer_id, ...)
		...
		if KraggyHUD:getSetting({"CustomHUD", "TEAMMATE", "LATENCY"}, 3) and  managers.network:session() then
...
elseif string.lower(RequiredScript) == "lib/managers/chatmanager" then
	if not KraggyHUD:getSetting({"HUDChat", "SPAM_FILTER"}, true) then return end
	...
	function ChatManager:_receive_message(channel_id, name, message, ...)
		...
		for _, pattern in ipairs(ChatManager._BLOCK_PATTERNS) do
			if message2:match("^" .. pattern .. "$") then
				return KraggyHUD.DEBUG_MODE and _receive_message_original(self, channel_id, name, "Pattern found: " .. pattern, ...)
			end
		end
