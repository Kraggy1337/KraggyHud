if string.lower(RequiredScript) == "lib/managers/hud/hudassaultcorner" then
	...
	function HUDAssaultCorner:update_banner_pos()
		...
		local banner_pos = math.clamp(KraggyHUD:getSetting({"AssaultBanner", "POSITION"}, 2), 1, 3)
		local assault_panel = self._hud_panel:child("assault_panel")
...
elseif string.lower(RequiredScript) == "lib/managers/localizationmanager" then
	...
	function LocalizationManager:hud_adv_assault()
		if KraggyHUD:getSetting({"AssaultBanner", "USE_ADV_ASSAULT"}, true) then
			if managers.hud and managers.hud:_locked_assault() then
				return self:text("KraggyHUD_locked_assault")
			else
				...
				if tweak and gai_state and assault_data and assault_data.active then
					...
					local phase = self:text("KraggyHUD_advassault_phase_title") .. "  " .. self:text("KraggyHUD_advassault_phase_" .. assault_data.phase)

					local spawns = get_value(gai_state, tweak.force_pool) * get_mult(gai_state, tweak.force_pool_balance_mul)
					local spawns_left = self:text("KraggyHUD_advassault_spawns_title") .. "  " .. math.round(math.max(spawns - assault_data.force_spawned, 0))
					...
					--end
					if time_left < 0 then
						time_left = self:text("KraggyHUD_advassault_time_overdue")
					else
						time_left = self:text("KraggyHUD_advassault_time_title") .. "  " .. string.format("%.2f", time_left)
					end
