if RequiredScript == "lib/units/enemies/cop/copdamage" then
	...
	function CopDamage:_process_popup_damage(attack_data)
		CopDamage.DMG_POPUP_SETTING = KraggyHUD:getSetting({"DamagePopup", "DISPLAY_MODE"}, 2)
	...
	function CopDamage:show_popup(damage, dead, headshot, color_id)
		if managers.waypoints then
			...
			local waypoint_color = color_id and ((color_id == 5 and KraggyHUD:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, false)) and KraggyHUD:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, Color.white) or tweak_data.chat_colors[color_id]) or KraggyHUD:getColorSetting({"DamagePopup", headshot and "HEADSHOT_COLOR" or "COLOR"}, "yellow")
			waypoint_color = waypoint_color:with_alpha(KraggyHUD:getSetting({"DamagePopup", "ALPHA"}, 1))
			local waypoint_duration = KraggyHUD:getSetting({"DamagePopup", "DURATION"}, 3)
			if waypoint and not waypoint:is_deleted() then
				...
				local params = {
					unit = self._unit,
					offset = Vector3(10, 10, KraggyHUD:getSetting({"DamagePopup", "HEIGHT"}, 20)),
					scale = 2 * KraggyHUD:getSetting({"DamagePopup", "SCALE"}, 1),
					...
					icon = {
						type = "icon",
						show = dead,
						scale = KraggyHUD:getSetting({"DamagePopup", "SKULL_SCALE"}, 1.2),
					...
					component_order = { KraggyHUD:getSetting({"DamagePopup", "SKULL_ALIGN"}, 1) == 1 and { "icon", "label" } or { "label", "icon" } , { "duration" } }
				}
