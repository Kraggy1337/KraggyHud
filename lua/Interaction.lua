if string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	PlayerStandard.NADE_TIMEOUT = KraggyHUD:getTweakEntry("STEALTH_NADE_TIMEOUT", "number", 0.25)		--Timeout for 2 NadeKey pushes, to prevent accidents in stealth
	...
	function PlayerStandard:_check_interaction_locked(t)
		PlayerStandard.LOCK_MODE = KraggyHUD:getSetting({"INTERACTION", "LOCK_MODE"}, 3)						--Lock interaction, if MIN_TIMER_DURATION is longer then total interaction time, or current interaction time
		PlayerStandard.MIN_TIMER_DURATION = KraggyHUD:getSetting({"INTERACTION", "MIN_TIMER_DURATION"}, 5)			--Min interaction duration (in seconds) for the toggle behavior to activate
	...
	function PlayerStandard:_check_interact_toggle(t, input)
		PlayerStandard.EQUIPMENT_PRESS_INTERRUPT = KraggyHUD:getSetting({"INTERACTION", "EQUIPMENT_PRESS_INTERRUPT"}, true) 	--Use the equipment key ('G') to toggle off active interactions
	...
	function PlayerStandard:_start_action_reload(t, ...)
		...
		PlayerStandard.SHOW_RELOAD = KraggyHUD:getSetting({"INTERACTION", "SHOW_RELOAD"}, false)
	...
	function PlayerStandard:_start_action_melee(t, input, instant, ...)
		...
		if not instant then
			PlayerStandard.SHOW_MELEE = KraggyHUD:getSetting({"INTERACTION", "SHOW_MELEE"}, false)
	...
	function PlayerStandard:_check_action_throw_grenade(t, input, ...)
		if input.btn_throw_grenade_press and KraggyHUD:getSetting({"INTERACTION", "SUPRESS_NADES_STEALTH"}, true) then
...
elseif string.lower(RequiredScript) == "lib/units/beings/player/states/playerdriving" then
	...
	function PlayerDriving:_check_interact_toggle(t, input)
		PlayerDriving.EQUIPMENT_PRESS_INTERRUPT = KraggyHUD:getSetting({"INTERACTION", "EQUIPMENT_PRESS_INTERRUPT"}, true) 	--Use the equipment key ('G') to toggle off active interactions
		local interrupt_key_press = input.btn_interact_press
	...
	function PlayerDriving:_check_interaction_locked(t)
		PlayerDriving.LOCK_MODE = KraggyHUD:getSetting({"INTERACTION", "LOCK_MODE"}, 3)						--Lock interaction, if MIN_TIMER_DURATION is longer then total interaction time, or current interaction time
		PlayerDriving.MIN_TIMER_DURATION = KraggyHUD:getSetting({"INTERACTION", "MIN_TIMER_DURATION"}, 5)			--Min interaction duration (in seconds) for the toggle behavior to activate
		local is_locked = false
...
elseif string.lower(RequiredScript) == "lib/managers/hud/hudinteraction" then
	...
	function HUDInteraction:set_interaction_bar_width(current, total)
		...
		if HUDInteraction.SHOW_TIME_REMAINING then
			...
			local show = perc < 1
			local color = math.lerp(HUDInteraction.GRADIENT_COLOR_START, KraggyHUD:getColor(HUDInteraction.GRADIENT_COLOR_NAME, 0.4), perc)
			...
			if self._interact_time_bgs then
				KraggyHUD:setOutlineText(self._interact_time_bgs, text, show and HUDInteraction.SHOW_TIME_REMAINING_OUTLINE)
			end
		end
	end
	...
	function HUDInteraction:show_interaction_bar(current, total)
		...
		HUDInteraction.SHOW_LOCK_INDICATOR = KraggyHUD:getSetting({"INTERACTION", "SHOW_LOCK_INDICATOR"}, true)
		HUDInteraction.SHOW_TIME_REMAINING = KraggyHUD:getSetting({"INTERACTION", "SHOW_TIME_REMAINING"}, true)
		HUDInteraction.SHOW_TIME_REMAINING_OUTLINE = KraggyHUD:getSetting({"INTERACTION", "SHOW_TIME_REMAINING_OUTLINE"}, false)
		HUDInteraction.SHOW_CIRCLE 	= KraggyHUD:getSetting({"INTERACTION", "SHOW_CIRCLE"}, true)
		HUDInteraction.LOCK_MODE = PlayerStandard.LOCK_MODE or 1
		HUDInteraction.GRADIENT_COLOR_NAME = KraggyHUD:getSetting({"INTERACTION", "GRADIENT_COLOR"}, "light_green")
		HUDInteraction.GRADIENT_COLOR_START = KraggyHUD:getColorSetting({"INTERACTION", "GRADIENT_COLOR_START"}, "white")
		...
		if HUDInteraction.SHOW_TIME_REMAINING then
			local fontSize = 32 * (self._circle_scale or 1) * KraggyHUD:getSetting({"INTERACTION", "TIMER_SCALE"}, 1)
			...
			if not self._interact_time_bgs then
				self._interact_time_bgs = KraggyHUD:makeOutlineText(self._hud_panel, {
					visible = false,
					text = "",
					valign = "center",
					align = "center",
				...
				KraggyHUD:setOutlineFontSize(self._interact_time_bgs, fontSize)
			end
			KraggyHUD:refreshOutlinePos(self._interact_time_bgs, self._interact_time)
			KraggyHUD:setOutlineText(self._interact_time_bgs, text, HUDInteraction.SHOW_TIME_REMAINING_OUTLINE)
		end
	...
	function HUDInteraction:hide_interaction_bar(complete, ...)
		...
		if self._interact_time_bgs then
			KraggyHUD:setOutlineText(self._interact_time_bgs, "", false)
		end
	...
	function HUDInteraction:set_locked(status, tweak_entry)
		...
		if status then
			...
			local locked_text = ""
			if KraggyHUD:getSetting({"INTERACTION", "SHOW_INTERRUPT_HINT"}, true) then
				...
				locked_text = managers.localization:to_upper_text(tweak_entry == "corpse_alarm_pager" and "KraggyHUD_int_locked_pager" or "KraggyHUD_int_locked", {BTN_CANCEL = btn_cancel})
			end
	...
	function HUDInteraction:_rescale(circle_scale, text_scale)
		local circle_scale = circle_scale or KraggyHUD:getSetting({"INTERACTION", "CIRCLE_SCALE"}, 0.8)
		local text_scale = text_scale or KraggyHUD:getSetting({"INTERACTION", "TEXT_SCALE"}, 0.8)
		local interact_text = self._hud_panel:child(self._child_name_text)
		local invalid_text = self._hud_panel:child(self._child_ivalid_name_text)
		local changed = false
...
elseif string.lower(RequiredScript) == "lib/managers/objectinteractionmanager" then
	ObjectInteractionManager.AUTO_PICKUP_DELAY = KraggyHUD:getTweakEntry("AUTO_PICKUP_DELAY", "number", 0.2)	 --Delay in seconds between auto-pickup procs (0 -> as fast as possible)
	...
	function ObjectInteractionManager:_update_targeted(player_pos, player_unit, ...)
		...
		if KraggyHUD:getSetting({"INTERACTION", "HOLD2PICK"}, true) and alive(self._active_unit) and not self._active_object_locked_data then
